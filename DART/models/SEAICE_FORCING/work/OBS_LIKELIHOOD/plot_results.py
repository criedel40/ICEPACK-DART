...
files1 = sorted(glob.glob('EaKF/assim_output_EaKF_*.nc'))
files2 = sorted(glob.glob('RHF/assim_output_RHF_*.nc'))
files3 = sorted(glob.glob('BOUNDED_RHF/assim_output_BoundedRHF_*.nc'))

EaKF = np.zeros((len(files1),ind.shape[0],99))
RHF = np.zeros((len(files2),ind.shape[0],99))
BoundedRHF = np.zeros((len(files3),ind.shape[0],99))

for f in range(0,len(files1)):
  print(files1[f])
  data = xr.open_dataset(files1[f])
  EaKF[f,:,:] = data.output_aice.values[ind,:]
  data.close()
  data = xr.open_dataset(files2[f])
  RHF[f,:,:] = data.output_aice.values[ind,:]
  data.close()
  data = xr.open_dataset(files3[f])
...
#
for m in range(0,99):
  ax[1].plot(tim,EaKF[1,:,m],'-',color='darkgrey',linewidth=0.5,alpha=0.5)
ax[1].plot(tim,EaKF[1,:,:].mean(axis=1),'-k',linewidth=1.5)
ax[1].plot(tim,truth,'-r',linewidth=1.5)
ax[1].set_title('EaKF - Gauss. Obs. Likelihood',loc='left',weight='bold',fontsize=12,pad=1.9)
##
for m in range(0,99):
  ax[2].plot(tim,RHF[1,:,m],'-',color='darkgrey',linewidth=0.5,alpha=0.5)
ax[2].plot(tim,RHF[1,:,:].mean(axis=1),'-k',linewidth=1.5)
ax[2].plot(tim,truth,'-r',linewidth=1.5)
ax[2].set_title('Rank Histogram Filter - Gauss. Obs. Likelihood',loc='left',weight='bold',fontsize=12,pad=1.1)
##
for m in range(0,99):
  ax[3].plot(tim,BoundedRHF[1,:,m],'-',color='darkgrey',linewidth=0.5,alpha=0.5)
ax[3].plot(tim,BoundedRHF[1,:,:].mean(axis=1),'-k',linewidth=1.5)
ax[3].plot(tim,truth,'-r',linewidth=1.5)
ax[3].set_title('Bounded Rank Histogram Filter - Gauss. Obs. Likelihood',loc='left',weight='bold',fontsize=12,pad=1.4)
####
...
plt.savefig('idealized_si_gauss.jpg',dpi=550,bbox_inches='tight')
os.system('open idealized_si_gauss.jpg')
##########
plt.rc('font', weight='bold')
tim = np.arange(0,dates.shape[0])
fig,ax = plt.subplots(4,1,sharex=True,figsize=(10,10))
