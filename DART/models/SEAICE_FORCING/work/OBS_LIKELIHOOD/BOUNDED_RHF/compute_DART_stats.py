...
for i in range(0,prior.shape[0]):
    bias_truth[i],bias_obs[i],rmse_truth[i],rmse_obs[i],totspr_truth[i],totspr_obs[i],rank_truth[i,:],rank_obs[i,:] = DART_functions.compute_obsspace_diag(prior[i,:].mean(),prior[i,:].var(ddof=1),truth[i],obs[i],obserr[i],prior[i,:],99,i,2)


new_file = netCDF4.Dataset('DART_obsspace_diags_BoundedRHF_gaus.nc','w')
