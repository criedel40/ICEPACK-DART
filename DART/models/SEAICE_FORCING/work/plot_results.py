...
ind = np.where((years>=2001)&(years<=2001))[0]
dates = dates[ind]
days = days[ind]
mons = mons[ind]
...
output_aice = data.input_aice.values[ind,:]
output_hi = data.input_hi.values[ind,:]

FO_sit = data.FO_SIT.values[ind,:]
obs_hi = data.obs_hi.values[ind]
######################
...
ax[1].set_title('Bounded RHF - SI Concentration',loc='left',weight='bold',fontsize=12,pad=1.9)
##
...
ax[1].set_title('Assimilation - SI Thickness',loc='left',weight='bold',fontsize=12,pad=1.9)
##
ax[1].set_xlim(tim[0],tim[-1])
ax[1].set_xticks(ind_mons)
ax[1].set_xticklabels(labels,fontsize=10,rotation=90)
for x in range(0,2):
  #ax[x].set_ylim(-0.1,3.0)
  #ax[x].set_yticks(np.arange(0.2,1.0+0.2,0.2))
  ax[x].tick_params(axis='y', labelsize=9)
  #ax[x].set_ylim(0,1.05)
  ax[x].grid(True, 'major', 'y', ls='--', lw=.5, c='k', alpha=.3,zorder=1)
