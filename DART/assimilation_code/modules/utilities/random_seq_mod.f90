...
!------------------------------------------------------------------------
!> return a random draw from a lower bound truncated normal
!> with the specified mean and standard deviation.

function random_truncnorm_a(r, mean, standard_deviation,a) 

type(random_seq_type), intent(inout) :: r
real(r8),              intent(in)    :: mean, standard_deviation,a
real(r8)                             :: random_truncnorm_a

real(r8) :: alpha,alpha_cdf,xi,xi_cdf,u
...
if (mean == a) then
  ...
  alpha = (a - mean)/standard_deviation
endif

alpha_cdf = norm_cdf(alpha,0.0_r8,1.0_r8)
u = ran_unif(r)
xi_cdf = alpha_cdf + u * (1.0_r8 - alpha_cdf)
call norm_inv(xi_cdf,xi)

random_truncnorm_a = mean + standard_deviation*xi

if (random_truncnorm_a < a) then
  print*,'Random value outside of bounds..STOP!'
  STOP
endif

end function random_truncnorm_a

!------------------------------------------------------------------------
!> return a random draw from a lower and upper bound truncated normal
!> with the specified mean and standard deviation.

function random_truncnorm_ab(r, mean, standard_deviation,a,b) 

type(random_seq_type), intent(inout) :: r
real(r8),              intent(in)    :: mean, standard_deviation,a,b
real(r8)                             :: random_truncnorm_ab

real(r8) :: alpha,alpha_cdf,xi,xi_cdf,u,beta_cdf,beta
...
if (mean == b) then
  ...
  beta = (b - mean)/standard_deviation
endif

alpha_cdf = norm_cdf(alpha,0.0_r8,1.0_r8)
beta_cdf = norm_cdf(beta,0.0_r8,1.0_r8)
u = ran_unif(r)

xi_cdf = alpha_cdf + u * ( beta_cdf - alpha_cdf )
call norm_inv(xi_cdf,xi)

random_truncnorm_ab = mean + standard_deviation*xi

if (random_truncnorm_ab < a .or. random_truncnorm_ab > b) then
  print*,'Random value outside of bounds..STOP!'
  STOP
endif

end function random_truncnorm_ab

!------------------------------------------------------------------------

!> return a random draw from a truncated gaussian distribution
!> with the specified mean and standard deviation.

function random_trunc_gaussian(r, mean, standard_deviation,a,b) 

type(random_seq_type), intent(inout) :: r
real(r8),              intent(in)    :: mean, standard_deviation,a,b
real(r8)                             :: random_trunc_gaussian
real(r8)                             :: alpha,beta,alpha_cdf,beta_cdf
real(r8) :: u,xi,xi_cdf
if ( .not. module_initialized ) call initialize_module
...
alpha_cdf = norm_cdf(a,mean,standard_deviation)
beta_cdf = norm_cdf(b, mean,standard_deviation)
...
random_trunc_gaussian = xi * standard_deviation + mean

end function random_trunc_gaussian
!------------------------------------------------------------------------
!> return a random draw from a truncated gaussian distribution
!> with the specified mean and standard deviation.

function random_beta(r, mean, standard_deviation) 

type(random_seq_type), intent(inout) :: r
real(r8),              intent(in) :: mean
real(r8),              intent(in)    :: standard_deviation
real(r8)                             :: random_beta,variance
real(r8)                             :: alpha,beta,u1,u2,w,v
real(r8) :: u,xi,xi_cdf,gamma,a,b,bot,tmp,p,g1,g2,mu
logical :: check_itr
...
if (mean == 1.0_r8) then
  random_beta = 99_r8
  do while (random_beta < 0.0_r8 .or. random_beta > 1.0_r8)
    random_beta = random_gaussian(r, mean, standard_deviation)
  end do
  return
else if (mean == 0.0_r8) then
  random_beta = 99_r8
  do while (random_beta < 0.0_r8 .or. random_beta > 1.0_r8)
    random_beta = random_gaussian(r, mean, standard_deviation)
  end do
...
if (variance > mu*(1.0_r8-mu)) then
  mu = 0.5_r8*(sqrt(1.0_r8 - 4.0_r8*(variance+1.0d-5)) + 1.0_r8)
endif  

a = mu*(((mu*(1.0_r8-mu))/variance) - 1.0_r8)
b = (1.0_r8-mu)*(((mu*(1.0_r8-mu))/variance) - 1.0_r8)

g1 = ran_gamma (r, a, 1.0_r8)
g2 = ran_gamma (r, b, 1.0_r8)
  
if (g1 == 0.0_r8 .or. g2 == 0.0_r8) then
  mu = 0.5_r8*(sqrt(1.0_r8 - 4.0_r8*(variance*2.0_r8)) + 1.0_r8)
  a = mu*(((mu*(1.0_r8-mu))/variance) - 1.0_r8)
  b = (1.0_r8-mu)*(((mu*(1.0_r8-mu))/variance) - 1.0_r8)
  g1 = ran_gamma (r, a, 1.0_r8)
  g2 = ran_gamma (r, b, 1.0_r8)
  if (g1 == 0 .or. g2 == 0) then
    p = a/(a+b)
    if (ran_unif(r) < p) then
      random_beta = 1.0_r8
    else
      random_beta = 0.0_r8
    endif
  else
    random_beta = g1/(g1+g2)
  endif
else
  random_beta = g1/(g1+g2)
endif
