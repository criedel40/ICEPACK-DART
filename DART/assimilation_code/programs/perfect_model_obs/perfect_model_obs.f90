...
use      obs_def_mod,     only : obs_def_type, get_obs_def_error_variance, get_obs_def_time, &
                                 get_obs_def_type_of_obs,set_obs_def_error_variance
...
use   random_seq_mod,     only : random_seq_type, init_random_seq, random_gaussian, &
                                 random_truncnorm_ab,random_truncnorm_a,random_beta
...
!use model_mod, only : spline_sic_err, compute_spline, &
!                      spline_sit_err
...
integer, allocatable    :: check_goodbad(:)
real(r8),allocatable    :: sicvals(:),sicerr(:),coefsicb(:),coefsicc(:),coefsicd(:)
real(r8),allocatable    :: sitvals(:),siterr(:),coefsitb(:),coefsitc(:),coefsitd(:)
...
integer,parameter       :: n_sit = 47
integer(i8)             :: model_size

integer                 :: istatus(1)

real(r8)                :: true_obs(1), obs_value(1), qc(1),sic_obs_err,sit_obs_err
real(r8)                :: slope,y_inter,computed_error
...
AdvanceTime: do
   ...
   allocate(check_goodbad(num_obs_in_set))
   allocate(sicvals(n_sic))
   allocate(sicerr(n_sic))
   allocate(coefsicb(n_sic))
   allocate(coefsicc(n_sic))
   allocate(coefsicd(n_sic))
   allocate(sitvals(n_sit))
   allocate(siterr(n_sit))
   allocate(coefsitb(n_sit))
   allocate(coefsitc(n_sit))
   allocate(coefsitd(n_sit))
   check_goodbad(:) = 0

   !initialize sic error spline
   !call spline_sic_err(sicvals,sicerr,coefsicb,coefsicc,coefsicd)
   !call spline_sit_err(sitvals,siterr,coefsitb,coefsitc,coefsitd)
   ...
   do j = 1, fwd_op_ens_handle%my_num_vars
      ...
      !print*,j,get_obs_def_type_of_obs(obs_def),true_obs(1)
      if (get_obs_def_type_of_obs(obs_def) == 12 .and. true_obs(1) > 1.0) then
        istatus = 5
        check_goodbad(j) = 1
      elseif (get_obs_def_type_of_obs(obs_def) == 12 .and. true_obs(1) < 0.0) then
        istatus = 5
        check_goodbad(j) = 1
      elseif (get_obs_def_type_of_obs(obs_def) == 15) then
        if (true_obs(1) < 0.0 .or. check_goodbad(j-1) == 1) then 
          istatus = 5 
        endif
      elseif (get_obs_def_type_of_obs(obs_def) == 16) then
        if (true_obs(1) < 0.0_r8 .or. check_goodbad(j-2) == 1) then
          istatus = 5
        endif
      endif

      if (get_obs_def_type_of_obs(obs_def) == 12) then
        if (true_obs(1) < 0.01 .and. true_obs(1) > 0.0) then
          istatus = 5
          check_goodbad(j) = 1
        endif
      elseif (get_obs_def_type_of_obs(obs_def) == 15) then
        if (check_goodbad(j-1) == 1) then
          istatus = 5
        endif
      elseif (get_obs_def_type_of_obs(obs_def) == 16) then
        if (check_goodbad(j-2) == 1) then
          istatus = 5
        endif
      endif
   ...
   if(my_task_id() == 0) then
      ...
      do i = 1, fwd_op_ens_handle%num_vars
         ...
         if (get_obs_def_type_of_obs(obs_def) == 12) then
           !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
           !if (true_obs(1) == 0.0_r8) then
           !  computed_error = 0.01*0.15
           !else
           !  computed_error = true_obs(1)*0.15
           !endif
           !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
           computed_error = 0.075_r8
           call set_obs_variance(obs,(computed_error*computed_error))
           call get_obs_def(obs, obs_def)
         elseif (get_obs_def_type_of_obs(obs_def) == 15) then
           !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
           !if (true_obs(1) == 0.0_r8) then
           computed_error = 0.1_r8**2
           !else
           !  computed_error = 0.1_r8*0.1_r8
           !endif
           !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
           call set_obs_variance(obs,(computed_error))
           call get_obs_def(obs, obs_def)
         elseif (get_obs_def_type_of_obs(obs_def) == 16) then
           !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
           !if (true_obs(1) == 0.0_r8) then
           !  computed_error = 0.01*0.1_r8
           !else
           !  computed_error = true_obs(1)*0.1_r8
           !endif
           !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
           computed_error = 0.035_r8
           call set_obs_variance(obs,(computed_error*computed_error))
           call get_obs_def(obs, obs_def)
         elseif (get_obs_def_type_of_obs(obs_def) == 10) then
           computed_error = 1.5_r8
           !if (computed_error < 0.005_r8) then
           !  computed_error = 0.005_r8
           !endif
           call set_obs_variance(obs,(computed_error*computed_error))
           call get_obs_def(obs, obs_def)
         endif
         ...
         if( qc_ens_handle%vars(i, 1) == 0 ) then
           !compute sic error 
           if (get_obs_def_type_of_obs(obs_def) == 12) then
             obs_value(1) = random_truncnorm_ab(random_seq, true_obs(1), &
                 sqrt(get_obs_def_error_variance(obs_def)),0.0_r8,1.0_r8)
             !computed_error = obs_value(1)*0.15_r8
             !call set_obs_variance(obs,(computed_error*computed_error))
             !call get_obs_def(obs, obs_def)
             !%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
             !print*,true_obs(1),sqrt(get_obs_def_error_variance(obs_def))
             !print*,true_obs(1),get_obs_def_error_variance(obs_def)
             !obs_value(1) = random_beta(random_seq, true_obs(1), &
             !    sqrt(get_obs_def_error_variance(obs_def)))
             if (obs_value(1) > 1.0_r8 .or. obs_value(1) < 0.0_r8) then
                 msgstring = 'Output from random truncated gaussian AB funcation is above 1 or below 0....STOP!'
                 call error_handler(E_ERR,'random_trunc_gaussian_ab',msgstring,source,revision,revdate)
             endif
           elseif (get_obs_def_type_of_obs(obs_def) == 15) then
             obs_value(1) = random_truncnorm_a(random_seq, true_obs(1), &
                 sqrt(get_obs_def_error_variance(obs_def)),0.0_r8)
             if (obs_value(1) < 0.0_r8) then
                 msgstring = 'Output from random truncated gaussian A funcation is below 0....STOP!'
                 call error_handler(E_ERR,'random_trunc_gaussian_a',msgstring,source,revision,revdate)
             endif
           elseif (get_obs_def_type_of_obs(obs_def) == 16) then
             obs_value(1) = random_truncnorm_a(random_seq, true_obs(1), &
                       sqrt(get_obs_def_error_variance(obs_def)),0.0_r8)
