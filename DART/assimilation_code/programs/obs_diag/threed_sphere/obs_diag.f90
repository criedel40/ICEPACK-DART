...
use   random_seq_mod, only : random_seq_type, init_random_seq, &
                             several_random_gaussians,random_truncnorm_ab, &
                             random_truncnorm_a
...
!character(len=stringlength), dimension(10) :: bounded_qtys = 'SAT_SEAICE_AGREG_CONCENTR'
!character(len=stringlength), dimension(10) :: bounded_type = 'DOUBLE'
...
logical :: work_bounded_dist = .false.
...
! COMMENT      qc_value = 2
         ...
         if ( create_rank_histogram ) then
            call get_obs_values(observation, copyvals)
            if (work_bounded_dist) then
              rank_histogram_bin = Rank_Histogram(copyvals, obs_index, obs_error_variance, obsname)
            else
...
if (num_trusted == MaxTrusted) then
   ...
   call error_handler(E_WARN, 'CountTrustedObsTypes', string1, source, text2=string2, text3=string3)
endif
...
function Rank_Histogram(copyvalues, obs_index, &
                    error_variance ,obsname) result(rank)
...
character(len=stringlength), optional :: obsname
integer                            :: rank
...
real(r8), dimension(ens_size) :: hold_vals(ens_size)
integer :: e
...
enddo
if (.not. use_zero_error_obs) then
  if (work_bounded_dist) then
    if (trim(obsname) == 'SAT_SEAICE_AGREG_CONCENTR') then
      do e=1, ens_size
        hold_vals(e) = random_truncnorm_ab(ran_seq,ensemble_values(e),stddev,0.0_r8,1.0_r8)
      enddo
      ensemble_values = sort(hold_vals)
    else if (trim(obsname) == 'SAT_SEAICE_AGREG_THICKNESS' .or. trim(obsname) == 'SAT_SEAICE_AGREG_SNOWDEPTH') then
      do e=1, ens_size
        hold_vals(e) = random_truncnorm_a(ran_seq,ensemble_values(e),stddev,0.0_r8)
      enddo
      ensemble_values = sort(hold_vals)
    else
