...
!               QTY_SEAICE_AGREG_THICKNESS, expected_obs, istatus)
!   case(SAT_SEAICE_AGREG_SNOWDEPTH)
!      call get_expected_agreg_thickness(state_handle, ens_size, location, &
!               QTY_SEAICE_AGREG_SNOWDEPTH, expected_obs, istatus)
...
real(r8)  :: loc_array(3),llat,llon,agreg_grid
logical :: thick_flag
real(r8)  :: grid(ens_size),grid2(ens_size)
integer :: icat
...
do icat = 1, Ncat
   location_fake = set_location(llat,llon,real(icat,r8),VERTISLEVEL)
   call interpolate(state_handle, ens_size, location_fake, obstype, grid ,istatus1,thick_flag)
   if (thick_flag) then
     where(istatus1 /= 0) istatus = istatus1
     where(istatus == 0) agreg_thickness = grid
     exit
   endif
   where(istatus1 /= 0) istatus = istatus1
   where(istatus == 0) agreg_volume = agreg_volume + grid   
   !!!!
   location_fake = set_location(llat,llon,real(icat,r8),VERTISLEVEL)
   call interpolate(state_handle, ens_size, location_fake, QTY_SEAICE_CONCENTR, grid2 ,istatus2)
   where(istatus2 /= 0) istatus = istatus2
   where(istatus == 0) agreg_sic = agreg_sic + grid2
end do
if (.not. thick_flag) then
  where(istatus1 /= 0) istatus = istatus1
  where(istatus2 /= 0) istatus = istatus2
